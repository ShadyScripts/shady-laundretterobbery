...
local function smashVitrine(k)
    ...
    QBCore.Functions.TriggerCallback('shady-laundretterobbery:server:getCops', function(cops)
        if cops >= Config.RequiredCops then
            ...
            QBCore.Functions.Progressbar('smash_vitrine', Lang:t('info.progressbar'), Config.WhitelistedWeapons[pedWeapon]['timeOut'], false, true, {
                ...
                TriggerServerEvent('shady-laundretterobbery:server:vitrineReward', k)
                TriggerServerEvent('shady-laundretterobbery:server:setTimeout')
                TriggerServerEvent('police:server:policeAlert', 'Robbery in progress')
                smashing = false
                TaskPlayAnim(ped, animDict, 'exit', 3.0, 3.0, -1, 2, 0, 0, 0, 0)
            end, function() -- Cancel
                TriggerServerEvent('shady-laundretterobbery:server:setVitrineState', 'isBusy', false, k)
                smashing = false
                TaskPlayAnim(ped, animDict, 'exit', 3.0, 3.0, -1, 2, 0, 0, 0, 0)
            end)
            TriggerServerEvent('shady-laundretterobbery:server:setVitrineState', 'isBusy', true, k)
...
RegisterNetEvent('QBCore:Client:OnPlayerLoaded', function()
    QBCore.Functions.TriggerCallback('shady-laundretterobbery:server:getVitrineState', function(result)
        Config.Locations = result
    end)
end)

RegisterNetEvent('shady-laundretterobbery:client:setVitrineState', function(stateType, state, k)
...
CreateThread(function()
    ...
    AddTextComponentSubstringPlayerName('shady laundrette')
...
CreateThread(function()
    if Config.UseTarget then
        for k, v in pairs(Config.Locations) do
            exports['qb-target']:AddBoxZone('shady-laundretterobbery' .. k, v.coords, 1, 1, {
                name = 'shady-laundretterobbery' .. k,
                heading = 40,
                minZ = v.coords.z - 1,
                maxZ = v.coords.z + 1,
                debugPoly = false
